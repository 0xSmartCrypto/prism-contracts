...
fn test_deposit_rewards_native() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: COLLECTOR.to_string(),
                msg: to_binary(&ExecuteMsg::PrismToYassetSwapHook {}).unwrap(),
                funds: vec![],
            }))
        ]
    );
...
fn test_deposit_rewards_token() {
    ...
    assert_eq!(
        ...
        vec![
            SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: reward_assets[0].info.to_string(),
    ...
    let reward_assets = vec![
        ...
        Asset {
            info: AssetInfo::Token {
                contract_addr: Addr::unchecked("MIR"),
            },
            amount: Uint128::from(500u128),
        },
    ];
    ...
    assert_eq!(
        ...
        vec![
            ...
            SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: reward_assets[1].info.to_string(),
                msg: to_binary(&Cw20ExecuteMsg::TransferFrom {
                    owner: info.sender.to_string(),
                    recipient: mock_env().contract.address.to_string(),
...
fn test_deposit_rewards_no_convert() {
    // testing depositing prism, no need for ConvertAndSend message
    let mut deps = mock_dependencies(&[]);
    init(&mut deps);

    // test depositing prism
    let reward_assets = vec![Asset {
        info: AssetInfo::Token {
            contract_addr: Addr::unchecked(PRISM_TOKEN),
        },
        amount: Uint128::from(1000u128),
    }];
    ...
    // test depositing yasset
    let reward_assets = vec![Asset {
        info: AssetInfo::Token {
            contract_addr: Addr::unchecked(YASSET_TOKEN),
        },
        amount: Uint128::from(1000u128),
    }];
...
#[test]
fn test_prism_to_yasset_swap_hook() {
    // testing depositing prism, no need for ConvertAndSend message
    let mut deps = mock_dependencies(&[]);
    init(&mut deps);

    let msg = ExecuteMsg::PrismToYassetSwapHook {};
    ...
    // no prism balance, nothing to do
    let info = mock_info(MOCK_CONTRACT_ADDR, &[]);
    let res = execute(deps.as_mut(), mock_env(), info.clone(), msg.clone()).unwrap();
    assert!(res.messages.is_empty());
    assert!(res.attributes.is_empty());

    // set prism balance to 1000
    deps.querier.with_token_balances(&[(
    ...
    assert_eq!(
        res.messages,
        vec![SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: PRISM_YASSET_PAIR.to_string(),
    ...
    assert_eq!(
        res.attributes,
        vec![attr("action", "prism_to_yasset_swap_hook"),]
    );
}
