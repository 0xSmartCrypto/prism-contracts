...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::PrismToYassetSwapHook {} => prism_to_yasset_swap_hook(deps, env, info),
...
pub fn deposit_rewards(
    ...
    // build list of assets that we need to swap to prism
    let assets_to_swap: Vec<Asset> = assets
        .into_iter()
        .filter(|asset| match asset.info.clone() {
            AssetInfo::Token { contract_addr } => {
                contract_addr != cfg.prism_token && contract_addr != cfg.yasset_token
            }
            _ => true,
        })
        .collect();

    // collector ConvertAndSend will convert to prism
    ...
    // yasset swap hook will finish conversion from prism to yasset
    messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: cfg.collector.to_string(),
        msg: to_binary(&ExecuteMsg::PrismToYassetSwapHook {})?,
        funds: vec![],
    }));
...
pub fn prism_to_yasset_swap_hook(
    deps: DepsMut,
    env: Env,
    info: MessageInfo,
) -> ContractResult<Response> {
    ...
    let amount = query_token_balance(&deps.querier, cfg.prism_token.clone(), env.contract.address)?;
    ...
    Ok(Response::new()
        .add_message(CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: cfg.prism_yasset_pair.to_string(),
        ...
        .add_attribute("action", "prism_to_yasset_swap_hook"))
}
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    Ok(ConfigResponse {
        ...
        prism_yasset_pair: cfg.prism_yasset_pair.to_string(),
...
pub fn _query_state(deps: Deps, env: Env, cfg: &Config) -> StdResult<StateResponse> {
    ...
    let xyasset_supply = query_supply(&deps.querier, cfg.xyasset_token.clone())?;
