...
use cosmwasm_std::{
    attr, to_binary, Addr, CosmosMsg, Decimal, Deps, DepsMut, Env, Response,
...
use crate::query::{
    query_lp_burn_rewards, query_pending_generator_rewards, query_pool_info,
};
...
pub fn bond(
    ...
    if lp_info.amt_lp > Uint128::zero() {
        clp_to_mint *= lp_info.amt_clp.clone()/lp_info.amt_lp.clone();
    }
    lp_info.amt_lp += amount;
    lp_info.amt_clp += clp_to_mint.clone();
...
pub fn unbond(
    ...
    let lp_contract = lp_info.lp_contract.clone();
    ...
    if lp_info.amt_lp > Uint128::zero() {
        clp_to_burn *= lp_info.amt_clp.clone()/lp_info.amt_lp.clone();
    }
    lp_info.amt_lp = lp_info.amt_lp.checked_sub(amount)?;
    lp_info.amt_clp = lp_info.amt_clp.checked_sub(clp_to_burn.clone())?;
    LP_INFO.save(deps.storage, &lp_info)?;
    ...
    messages.push(
        CosmosMsg::Wasm(WasmMsg::Execute {
            ...
            msg: to_binary(&Cw20ExecuteMsg::BurnFrom {
                owner: sender_addr.clone().to_string(),
                amount: clp_to_burn,
            })?,
            funds: vec![],
        })
...
pub fn update_rewards(
    deps: Deps,
    env: Env,
) -> StdResult<(Decimal, Uint128, Vec<CosmosMsg>)> {
    let config = CONFIG.load(deps.storage)?;
    let mut lp_info = LP_INFO.load(deps.storage)?;
    ...
    let inv_last_liquidity = decimal_division(Uint128::new(1), lp_info.last_liquidity.clone());
    let lp_to_burn =
        (Uint128::new(1).checked_sub(inv_new_liquidity / inv_last_liquidity)?) * lp_info.amt_lp.clone();

    // query generator reward
    let pending_gen_rewards = query_pending_generator_rewards(
        deps,
        env,
        &deps.querier,
    )?;
    ...
    // query amm reward
    let pending_amm_rewards = query_lp_burn_rewards(
        deps,
        &deps.querier,
        lp_to_burn,
    )?;
